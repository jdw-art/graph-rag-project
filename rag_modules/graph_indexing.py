...
class GraphIndexingModule:
    ...
    def create_entity_key_values(self, recipes: List[Any], ingredients: List[Any],
        ...
        :param cooking_steps: 制作步骤
        :return: 实体键值对
        """
        logger.info("开始构建实体键值对...")
        ...
        for recipe in recipes:
            ...
            # 构建详细内容
            content_parts = [f"菜谱名称: {entity_name}"]
        ...
        for ingredient in ingredients:
            ...
            self.entity_kv_store[entity_id] = entity_ky
            self.key_to_entities[entity_name].append(entity_id)

        # 处理烹饪步骤实体
        for step in cooking_steps:
            entity_id = step.node_id
            entity_name = f"烹饪步骤_{entity_id}"

            content_parts = [f"烹饪步骤_{entity_name}"]
    ...
    def _generate_relation_index_keys(self, source_entity: EntityKeyValue, target_entity: EntityKeyValue,
    ...
    def deduplicate_entities_relations(self):
        ...
        logger.info(f"去重完成，删除了 {entities_to_remove} 个重复实体，{relations_to_remove} 个重复关系")

    def _rebuild_key_mapping(self):
        """
        重建键到实体/关系的映射
        :return:
        """
