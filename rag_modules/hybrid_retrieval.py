...
class HybridRetrievalModule:
    ...
    def _build_graph_index(self):
        ...
        try:
            ...
            # 去重优化
            self.graph_indexing.deduplicate_entities_relations()

            self.graph_indexed = True
            stats = self.graph_indexing.get_statistics()
            logging.info(f"图索引构建完成: {stats}")
    ...
    def _extract_relationships_from_graph(self):
        ...
        try:
            with self.driver.session() as session:
                ...
                RETURN source.nodeId as source_id, type(r) as relation_type, target_nodeId as target_id
                LIMIT 1000
                """
    ...
    def _get_node_neighbors(self, node_id: int, max_neighbors: int = 3) -> List[str]:
        """获取节点的邻居信息"""
        try:
            with self.driver.session() as session:
                query = """
    ...
    def hybrid_retrieval(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        def vector_search():
            ...
            try:
                ...
                logger.info(f"向量检索成功: {len(vector_docs)} 个结果")
            except Exception as e:
        ...
        for i in range(max_len):
            ...
            if i < len(dual_docs):
                ...
                if doc_id not in seen_doc_ids:
                    ...
                    doc.metadata["search_method"] = "dual_search"
            ...
            if i < len(vector_docs):
                ...
                if doc_id not in seen_doc_ids:
                    seen_doc_ids.add(doc_id)
                    doc.metadata["search_method"] = "vector_search"
                    doc.metadata["round_robin_order"] = len(merged_docs)
