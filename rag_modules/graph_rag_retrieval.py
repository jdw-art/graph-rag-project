...
class GraphRagRetrievalModule:
    ...
    def understand_graph_query(self, query: str) -> GraphQuery:
        ...
        """
        prompt = f"""
        作为图数据库专家，分析一下查询的图结构意图
        
        查询：{query}
        
        请识别：
        1. 查询类型：
           - entity_relation: 询问实体间的直接关系（如鸡肉和胡萝卜能一起做菜吗？）
    ...
    def adaptive_query_planing(self, query: str) -> List[GraphQuery]:
        """
    ...
    def graph_rag_search(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        try:
            ...
            if graph_query.query_type in [QueryType.MULTI_HOP, QueryType.PATH_FINDING]:
                ...
                results.extend(self._path_to_documents(paths, query))
    ...
    def _path_to_documents(self, paths: List[GraphPath], query: str) -> List[Document]:
        """将图路径转换为Document对象"""
        documents = []
        ...
        for i, path in enumerate(paths):
            ...
            doc = Document(
                ...
                metadata={
                    ...
                    "relevance": path.relevance_score,
