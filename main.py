...
from rag_modules.intelligent_query_router import IntelligentQueryRouterModule
from rag_modules.recipe_recommendation import RecipeRecommendationModule
from rag_modules.session_cache_manager import SessionCacheManager
from rag_modules.web_service_handler import WebServiceHandlerModule
...
class AdvancedGraphRAGSystem:
    ...
    def initialize_system(self):
        ...
        try:
            ...
            # 5. 图RAG检索模块
            print("初始化图RAG检索引擎...")
            self.graph_rag_retrieval = GraphRagRetrievalModule(
                config=self.config,
                llm_client=self.generation_module.client
            )
            ...
            # 6. 智能插叙路由器
            print("初始化智能查询路由器...")
            self.query_router = IntelligentQueryRouterModule(
            ...
            # 8. 菜谱推荐管理器
            print("初始化菜谱推荐管理器...")
            self.recipe_manager = RecipeRecommendationModule()

            # 9. Web服务处理器
            print("初始化Web服务处理器...")
            self.web_handler = WebServiceHandlerModule(self)
        ...
        try:
            ...
            if self.index_module.has_collection():
                ...
                if self.index_module.load_collection():
                    ...
                    # 重要：即使从已存在的数据库加载，也需要加载图数据以支持图索引
                    print("加载图数据以支持图索引...")
    ...
    def ask_question_without_routing(self, question: str, stream: bool = False, explain_routing: bool = False):
        """
        智能回答：自动选择最佳检索策略
        :param question:
        :param stream:
        ...
        try:
            ...
            # 5. 性能统计
            end_time = time.time()
            print(f"\n 问题回答完成，耗时：{end_time - start_time: .2f}秒")
